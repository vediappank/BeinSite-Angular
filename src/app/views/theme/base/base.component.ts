...
export class BaseComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // register configs by demos
     console.log('BaseComponent:LayoutConfig:configs::::' + JSON.stringify(new LayoutConfig().configs));
    this.layoutConfigService.loadConfigs(new LayoutConfig().configs);
    new MenuConfig(authService).configs.subscribe(config => {
     console.log('BaseComponent:MenuConfig:configs::::' +  JSON.stringify(config) );
      this.menuConfigService.loadConfigs(config);
    });
    // this.menuConfigService.loadConfigs(new MenuConfig(authService).configs);
    this.pageConfigService.loadConfigs(new PageConfig().configs);

    // setup element classes
    // console.log('BaseComponent:LayoutConfig:configs222::::' + JSON.stringify(this.layoutConfigService.getConfig()));
  ...
  ngOnInit(): void {    
    ...
    // console.log('Content.Width: ' + objectPath.get(config, 'content.width'));
