...
export class DeleteEntityDialogComponent implements OnInit {
	...
	onYesClick(): void {
		...
		// Check condition HTML page if Textarea not shown
		...
		else if (this.data.buttonType === 'Add Comment') {
			if (this.data.cancelDescription == '') {
				this.ErrorMessage = 'Enter Comment Text';
				this.hasFormErrors= true;
				this.viewLoading = false;
				return;
			}
		...
		else if (this.data.buttonType === 'Add Reply') {
			if (this.data.cancelDescription == '') {
				this.ErrorMessage = 'Enter reply to the Ticket';
				this.hasFormErrors= true;
				this.viewLoading = false;
				return;
			}
		...
		else if (this.data.buttonType === 'Confirm') {
			// if (this.data.cancelDescription == '') {
 ...
 */
	isTitleValid(): boolean {
		...
		if (this.data.buttonType === 'Add Comment') { // Check condition HTML page if Textarea not shown
			return (this.data.cancelDescription > 0);
		}
		if (this.data.buttonType === 'Add Reply') { // Check condition HTML page if Textarea not shown
			return (this.data.cancelDescription > 0);
		}
		else
			return true;
