...
@Component({
  selector: 'kt-multibar-line-hiring-charts',
  templateUrl: './multibar-line-hiring-charts.component.html',
  styleUrls: ['./multibar-line-hiring-charts.component.scss']
})
export class MultibarLineHiringChartsComponent implements OnInit {
    ...
    for (const propName in changes) {
      if (propName === 'data') {
        ...
        if (change.currentValue) { 
              ...
              this.finReptWidgetData = change.currentValue.filter(item => item.week !== 'Week 99999-99999');
              console.log('loadHiringData Reports Data::::' + JSON.stringify(this.finReptWidgetData));
              this.loadHiringData(this.finReptWidgetData);
        }
      }
    }
  }
  ...
  loadHiringData(reportdata: any) {
    let chart = am4core.create("HiringCharts", am4charts.XYChart);
    ...
    valueAxis.min = 0;
    valueAxis.title.text = "Agents Count(#)";
    valueAxis.extraMax = 0.1;

    //Agent Left Axis
    var valueAxis2 = chart.yAxes.push(new am4charts.ValueAxis());
    valueAxis2.title.text = "Agent Left (%)";
    ...
    function createSeries(field, name, stacked) {
     ...
     if(field =="ActiveCount")
     {
        series.tooltipHTML = `
       
      <table style="color:white; " >
      <tr>
        <td align="left">Agent Left (%): </td>
        <td >{LeftPer}</td>
      </tr>
      <tr>
        <td align="left">Active Agent: </td>
        <td >{ActiveCount}</td>
      </tr>
      <tr>
        <td align="left">Joined Agent: </td>
        <td style="color:white;">{JoinedCount}</td>
      </tr>
      <tr>
        <td align="left">Left Agent: </td>
        <td style="color:white;">{LeftCount}</td>
      </tr>
      </table>
      `;
     }
    ...
    createSeries("ActiveCount", "Active Agent", false);
    createSeries("JoinedCount", "Joined Agent", false);
    createSeries("LeftCount", "Left Agent ", false);

    var series2 = chart.series.push(new am4charts.LineSeries());
    series2.dataFields.valueY = "LeftPer";
    series2.name = "Agent Left (%)";
