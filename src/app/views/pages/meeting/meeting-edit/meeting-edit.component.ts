...
export class MeetingEditComponent implements OnInit {
  ...
  hasdatevalidationErrors: boolean = false;
  ...
  SelectedAgentList: string;
	 ...
	 */
  constructor(public dialogRef: MatDialogRef<MeetingEditComponent>,
  ...
  ngOnInit() {
    ...
    this.meeting$.subscribe(res => {
      ...
      this.meeting.ApproverName = res.ApproverName;
  ...
  getAllSupervisor() {
    this.auth.GetAllSupervisosr().subscribe((_Super: any[]) => {
  ...
  assignSupervisor() {
    ...
    const role = find(this.getAllSupervisor, (_Supervisor: any) => {
  ...
  //  if (this.SelectedMeetingtype === 'One to One')
     // this.SelectedStatus = 'Apprvoved';
  //  else
      this.SelectedStatus = 'Pending-Approval';
  }
  //Auto Complete supervisor
  _filter(value: string): any[] {
    ...
    return this.unassignedSupervisor.filter(option => option.fullname.toLowerCase().includes(filterValue));
    }
    else
    {
      this.filteredOptions = this.myControl.valueChanges
      .pipe(
        startWith(''),
        map(value => this.unassignedSupervisor.filter(option => option.fullname.toLowerCase().includes(value))));
    }
  }
  ...
  prepareMeeting(): MeetingModel {
    ...
    let value: any;
    value = this.unassignedSupervisor.filter(option => option.fullname == this.myControl.value);
    const _meeting = new MeetingModel();
    if (localStorage.hasOwnProperty("currentUser")) {
      this.UserID = JSON.parse(localStorage.getItem('currentUser')).agentid;
    }
    if (value.length > 0)
      _meeting.usersupervisorid = value[0].id;
    else
    ...
    if (_meeting.Status == 'Approved')
      _meeting.Approved_By = this.UserID.toString();
    else
      _meeting.Approved_By = '';
    return _meeting;
  }
  ...
  onSubmit() {
    ...
    this.hasdatevalidationErrors = false;
    this.loadingAfterSubmit = false;
    if (!this.isTitleValid()) {
      this.hasFormErrors = true;
      this.hasdatevalidationErrors = true;
      return;
    }
    const editedMeeting = this.prepareMeeting();
    if (editedMeeting.Duration === 'Start Date should be greater then end date') {
      this.hasFormErrors = true;
      this.hasdatevalidationErrors = true;
      this.loadingAfterSubmit = true;
      this.ErrorMessage = "Please Check Your Meeting Start Date&Time and End Date&Time";
      return;
    }
    else {
      if (this.data.id > 0) {
  ...
  getTitle(): string {
    if (this.meeting && this.meeting.id) {
      ...
      return `Edit Meeting '${this.meeting.Subject}'`;
    }
    // tslint:disable-next-line:no-string-throw
    return 'New Meeting';
  }
  ...
  isTitleValid(): boolean {    
    let CurentDate = moment(new Date()).format('YYYY-MM-DD')
    ...
    // console.log('this.SelectedAgentList.length::::'+ JSON.stringify(this.SelectedAgentList));
    if (this.meeting && this.meeting.id) {
      return (this.meeting && this.meeting.Subject && this.meeting.Subject.length && this.meeting.Description && this.meeting.Description.length > 0
        ...
        && this.SelectedMeetingtype && this.SelectedMeetingtype.length > 0 && (this.SelectedAgentList != '' && this.SelectedAgentList != undefined)
        && sDate >= CurentDate
        && eeDate >= ssDate
      ...
      return (this.meeting && this.meeting.Subject && this.meeting.Subject.length && this.meeting.Description && this.meeting.Description.length > 0
        ...
        && sDate >= CurentDate
        && eeDate >= ssDate && !this.hasdatevalidationErrors
      );
    }
    //return true;
  }
  ...
  getduartion() {    
    ...
    let currentdatetime = moment(currentdate + ' ' + currenttime);

    if (moment(currentdatetime) > moment(ssDate)) {
      console.log('Start date and time should be greater then current date and time')
      this.MeetingDuration = 'Start date-time should be greater then current date and time';
      this.hasFormErrors = true;
      this.ErrorMessage = this.MeetingDuration;
      return this.MeetingDuration;
    }
    else if (moment(eeDate) < moment(ssDate)) {
      console.log('End date and time should be greater then Start date and time')
    ...
    // if (moment(ssDate) > moment(eeDate)) {
