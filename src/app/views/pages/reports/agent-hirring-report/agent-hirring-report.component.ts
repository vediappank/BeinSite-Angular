...
@Component({
  selector: 'kt-agent-hirring-report',
  templateUrl: './agent-hirring-report.component.html',
  styleUrls: ['./agent-hirring-report.component.scss']
})
export class AgentHirringReportComponent implements OnInit {

  public reportType: string = 'agent';
  public reportRange: any;
  public reportFields: any;
  public agentreportinputReq : AgentHirringRequest;
  public title = 'Agent Hirring Reports';
  private gridApi;
  ...
  {    
    ...
    if( this .reportRange.startDate && this .reportRange.endDate) {
     ...
     //
      this .agentData = this.auth.GetAgentHirringReport(this.agentreportinputReq);
      this .agentData.subscribe(_hirrringbarList => {
        this.ShowBarCharts(_hirrringbarList);  
        setTimeout(() => {this .autoSizeAll();}, 1000);
     console.log('Agent Hirrring Rrports Data Response Called::::' + JSON.stringify(_hirrringbarList));
      });
    }
  }
  autoSizeAll() {
