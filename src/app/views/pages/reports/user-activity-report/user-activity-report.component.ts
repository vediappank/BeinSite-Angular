...
import { UserActivityRequest } from '../_models/useractivityrequest.model';
...
@Component({
  selector: 'kt-user-activity-report',
...
export class UserActivityReportComponent implements OnInit {
  ...
  public agentreportinputReq: UserActivityRequest;
  public title = 'User Activity Report';
  private gridApi;
  ...
  refreshCols() {
    this.columnDefs = [
      ...
      { headerName: 'Activity', field: 'activity', width: 300 },
  ...
  getReportData() {    
    console.log('User Activity Calls Data Function Called');
    // if (this.reportRange.startDate && this.reportRange.endDate) {
      ...
      this.agentData = this.auth.GetUserActivityReport(this.agentreportinputReq);
      this.agentData.subscribe(_ratiobarList => {                 
        setTimeout(() => { this.autoSizeAll(); }, 1000);
        console.log('User Activity Calls Response Called::::' + JSON.stringify(_ratiobarList));
      });
    // }
  }
