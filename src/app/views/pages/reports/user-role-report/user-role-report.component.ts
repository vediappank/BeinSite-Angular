...
@Component({
  selector: 'kt-user-role-report',
  templateUrl: './user-role-report.component.html',
  styleUrls: ['./user-role-report.component.scss']
})
export class UserRoleReportComponent implements OnInit {
  ...
  getReportData() {    
    console.log('User Role Calls Data Function Called');
    // if (this.reportRange.startDate && this.reportRange.endDate) {
      ...
      this.agentData = this.auth.GetUserRoleReport(this.agentreportinputReq);
      this.agentData.subscribe(_ratiobarList => {                 
        setTimeout(() => { this.autoSizeAll(); }, 1000);
        console.log('User Role Calls Response Called::::' + JSON.stringify(_ratiobarList));
      });
    // }
  }
