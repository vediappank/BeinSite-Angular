...
import {UserRoleRequest } from '../_models/userrolerequest.model';
...
@Component({
  selector: 'kt-user-supervisor-report',
  templateUrl: './user-supervisor-report.component.html',
  styleUrls: ['./user-supervisor-report.component.scss']
})
export class UserSupervisorReportComponent implements OnInit {
  ...
  public agentreportinputReq: UserRoleRequest;
  public title = 'User Organization Report';
  private gridApi;
  ...
  refreshCols() {
    this.columnDefs = [
      ...
      { headerName: 'Supervisori ID', field: 'supervisorid', width: 160 },
      { headerName: 'Supervisor First Name', field: 'supervisorfirstname', width: 250 },
      { headerName: 'Supervisor Last Name', field: 'supervisorlastname', width: 250 },
  ...
  getReportData() {    
    console.log('User Supervisor Calls Data Function Called');
    // if (this.reportRange.startDate && this.reportRange.endDate) {
      ...
      this.agentData = this.auth.GetUserSupervisorReport(this.agentreportinputReq);
      this.agentData.subscribe(_ratiobarList => {                 
        setTimeout(() => { this.autoSizeAll(); }, 1000);
        console.log('User Supervisor Calls Response Called::::' + JSON.stringify(_ratiobarList));
      });
    // }
  }
