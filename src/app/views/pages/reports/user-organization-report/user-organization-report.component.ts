...
import {UserOrganizationRequest } from '../_models/userorganizationrequest.model';
...
@Component({
  selector: 'kt-user-organization-report',
  templateUrl: './user-organization-report.component.html',
  styleUrls: ['./user-organization-report.component.scss']
})
export class UserOrganizationReportComponent implements OnInit {
  ...
  public agentreportinputReq: UserOrganizationRequest;
  ...
  refreshCols() {
    this.columnDefs = [
      ...
      { headerName: 'First Name', field: 'firstname', width: 500 },
      { headerName: 'Last Name', field: 'lastname', width: 500 },
      { headerName: 'Organisation', field: 'organisationname', width: 500 }
  ...
  getReportData() {    
    console.log('Organisationname Calls Data Function Called');
    // if (this.reportRange.startDate && this.reportRange.endDate) {
      ...
      this.agentData = this.auth.GetUserOrganizationReport(this.agentreportinputReq);
      this.agentData.subscribe(_ratiobarList => {                 
        setTimeout(() => { this.autoSizeAll(); }, 1000);
        console.log('Organisationname Calls Response Called::::' + JSON.stringify(_ratiobarList));
      });
    // }
  }
