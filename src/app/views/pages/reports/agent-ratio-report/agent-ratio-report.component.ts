...
@Component({
  selector: 'kt-agent-ratio-report',
  templateUrl: './agent-ratio-report.component.html',
  styleUrls: ['./agent-ratio-report.component.scss']
})
export class AgentRatioReportComponent implements OnInit {

  public reportType: string = 'agent';
  public reportRange: any;
  public reportFields: any;
  public agentreportinputReq: AgentRatioRequest;
  public title = 'Agent Ratio Report';
  private gridApi;
  ...
  refreshCols() {
    this.columnDefs = [      
      { headerName: 'Role', field: 'RoleName', width: 250 },
      { headerName: 'Call Center', field: 'CCName', width: 250 },
      { headerName: 'Actual Count', field: 'ActualCount', width: 280, cellStyle: params => params.value < 0? {'background-color': 'gray', color:'#fff'} : {color: 'black'}},
      { headerName: 'Expected Ratio', field: 'ExpectedRatio', width: 280, cellStyle: params => params.value < 0 ? {'background-color': 'gray', color:'#fff'} : {color: 'black'}},
      { headerName: 'Expected Count', field: 'ExpectedCount', width: 300, cellStyle: params => params.value < 0 ? {'background-color': 'gray', color:'#fff'} : {color: 'black'}},
      { headerName: 'Deviation Count', field: 'DeviationCount', width: 300, cellStyle: params => params.value < 0 ? {'background-color': 'gray', color:'#fff'} : {color: 'black'}}
    ];
    if (this.gridApi) {
  ...
  getReportData() {
    ...
    if (this.reportRange.startDate && this.reportRange.endDate) {
      ...
      //
      this.agentData = this.auth.GetAgentRatioReport(this.agentreportinputReq);
