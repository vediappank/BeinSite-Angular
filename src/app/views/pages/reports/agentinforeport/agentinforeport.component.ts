...
@Component({
  selector: 'kt-agentinforeport',
  templateUrl: './agentinforeport.component.html',
  styleUrls: ['./agentinforeport.component.scss']
})
export class AgentinforeportComponent implements OnInit {

  public reportRange: any;
  public reportFields: any;
  public agentreportinputReq: AgentInfoRequest;
  public title = 'Agent Performance Report';
  ...
  refreshCols() {
    this.columnDefs = [
      ...
      { headerName: 'Supervisor First Name', field: 'SupervisorFirstName', width: 230 },
      { headerName: 'Supervisor Last Name', field: 'SupervisorLastName', width: 230 },
  ...
  getReportData() {    
    console.log('Agent Info Rrports Data Function Called');
    if (this.reportRange.startDate && this.reportRange.endDate) {
      ...
      //
      this.agentData = this.auth.GetAgentInfoReport(this.agentreportinputReq);
      this.agentData.subscribe(_ratiobarList => {
        setTimeout(() => { this.autoSizeAll(); }, 1000);
        console.log('Agent Info Rrports Data Response Called::::' + JSON.stringify(_ratiobarList));
      });
    }
  }
  redirecttoList() {
