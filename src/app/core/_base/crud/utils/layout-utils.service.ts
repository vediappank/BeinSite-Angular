...
export class LayoutUtilsService {
	 ...
	 */
	deleteElement(title: string = '', description: string = '', waitDesciption: string = '', buttonText: string = 'Delete', cancelDescription: string = '', buttonType: string = '') {
	
		let width: string;
		if (buttonType === 'Cancel Meeting' || buttonType === 'Completed Meeting' || buttonType === 'Add Comment' || buttonType === 'Add Reply')
			width = '750px'
		else
			width = '440px'

			// Check DeleteEntityDialogComponent's condition HTML page if Textarea still not shown
		return this.dialog.open(DeleteEntityDialogComponent, {
			data: { title, description, waitDesciption, buttonText, cancelDescription, buttonType },
			width: width
		});
	}
