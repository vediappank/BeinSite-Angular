...
import * as AuthReducer from '../_reducers/auth.reducers';
...
export const getLoggedInUser = createSelector(
    ...
    (auth: AuthReducer.State) => {
        console.log('Selector Authencation: ' + JSON.stringify(auth));
        return auth.user;
    }
);
...
export const isLoggedIn = createSelector(
    AuthReducer.getAuthState,
    (auth: AuthReducer.State) => auth.loggedIn
);
...
export const currentAuthToken = createSelector(
    AuthReducer.getAuthState,
    (auth: AuthReducer.State) => auth.authToken
);

export const isUserLoaded = createSelector(
    AuthReducer.getAuthState,
    (auth: AuthReducer.State) => auth.isUserLoaded
);
